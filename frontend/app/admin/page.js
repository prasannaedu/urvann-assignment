...
export default function Admin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`${API}/plants/${editId}`, formData, {
    ...
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col max-w-md mb-6">
        ...
        <div className="flex gap-2">
          ...
          {editId && <button type="button" onClick={resetForm} className="bg-gray-400 text-white p-2 rounded">Cancel</button>}
        </div>
      </form>
        ...
        {plants.map((plant) => (
          <div key={plant._id} className="border p-4 rounded shadow">
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={() => handleEdit(plant)} className="bg-yellow-500 text-white px-3 py-1 rounded">Edit</button>
              <button onClick={() => handleDelete(plant._id)} className="bg-red-500 text-white px-3 py-1 rounded">Delete</button>
            </div>
          </div>
        ))}
      </div>
